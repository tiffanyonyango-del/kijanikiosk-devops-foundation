import { hasOwnProperty, isBlockedPrototypeKey } from "./prototype-guard.mjs";
...
var debug = debugBase("velocity");
var posUnknown = {
	first_line: "unknown",
	first_column: "unknown"
};
var References = class extends Compile {
	/**
	* get variable value
	...
	*/
	getReferences(ast, isVal) {
		...
		if (hasOwnProperty(context, text)) return ast.prue && escape ? convert(context[text]) : context[text];
		...
		if (isVal && ret === void 0) ret = isSilent ? "" : getRefText(ast);
	...
	*/
	getPropMethod(property, baseRef, ast) {
		...
		try {
			...
			const err = `on ${getRefText(ast)} at L/N ${pos.first_line}:${pos.first_column}`;
