...
var LiteralCompiler = class extends require_base_compile.Compile {
	...
	*/
	getLiteral(literal) {
		...
		if (type === "string") return this.getString(literal);
		if (type === "integer") return parseInt(literal.value, 10);
		if (type === "decimal") return parseFloat(literal.value);
		if (type === "array") return this.getArray(literal);
		if (type === "map") {
			const ret = {};
	...
	*/
	getString(literal) {
		...
		if (literal.isEval && (val.indexOf("#") !== -1 || val.indexOf("$") !== -1)) return this.evalStr(val);
		return val;
	}
	/**
	...
	*/
	getArray(literal) {
		if (!("isRange" in literal && literal.isRange === true)) return literal.value.map((exp) => this.getLiteral(exp));
		const [begin, end] = literal.value.map((exp) => {
			if (typeof exp === "string") return parseInt(exp, 10);
			if (typeof exp === "number") return exp;
			return this.getReferences(exp);
		});
		if (Number.isNaN(begin) || Number.isNaN(end)) return [];
		let i = 0;
		const ret = [];
		if (begin < end) for (i = begin; i <= end; i++) ret.push(i);
		else for (i = begin; i >= end; i--) ret.push(i);
		return ret;
	}
};
require_utils.applyMixins(require_base_compile.Compile, [LiteralCompiler]);
//#endregion
