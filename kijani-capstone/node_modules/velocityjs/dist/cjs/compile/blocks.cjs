const require_utils = require("../utils.cjs");
const require_base_compile = require("./base-compile.cjs");
const require_parse = require("../parse.cjs");
const require_index = require("../helper/index.cjs");
//#region src/compile/blocks.ts
/**
* blocks such as if, foreach, macro syntax handler
*/
var BlockCommand = class extends require_base_compile.Compile {
	getBlock(block) {
		const ast = block[0];
		let ret = "";
		switch (ast.type) {
			case "if":
				ret = this.getBlockIf(block);
				break;
			case "foreach":
				ret = this.getBlockEach(block);
				break;
			case "macro":
				this.setBlockMacro(block);
				break;
			case "noescape":
				ret = this.renderAstList(block.slice(1));
				break;
			case "define":
				this.setBlockDefine(block);
				break;
			case "macro_body":
				ret = this.getMacroBody(block);
				break;
			default: ret = this.renderAstList(block);
		}
		return ret || "";
	}
	/**
	* define
	*/
	setBlockDefine(block) {
		const ast = block[0];
		this.defines[ast.id] = block.slice(1);
	}
	/**
	* define macro
	*/
	setBlockMacro(block) {
		const ast = block[0];
		this.macrosStore[ast.id] = {
			asts: block.slice(1),
			args: ast.args
		};
	}
	getMacroBody(asts) {
		const ast = asts[0];
		const _block = asts.slice(1);
		const bodyContent = this.eval(_block, {});
		return this.getMacro(ast, bodyContent);
	}
	/**
	* parse macro call
	*/
	getMacro(ast, bodyContent) {
		const macro = this.macrosStore[ast.id];
		if (!macro) {
			const fn = this.macros[ast.id];
			if (!fn || typeof fn !== "function") return "";
			const jsArgs = (ast.args || []).map((arg) => this.getLiteral(arg));
			try {
				return fn.apply(this.macros, jsArgs);
			} catch (e) {
				const pos = ast.pos;
				const err = `\n      at ${require_index.getRefText(ast)} L/N ${pos.first_line}:${pos.first_column}`;
				if (e instanceof Error) {
					e.name = "";
					e.message += err;
				}
				throw e;
			}
		}
		const { args, asts } = macro;
		const callArgs = ast.args;
		const local = { bodyContent };
		const contextId = `macro:${ast.id}:${require_utils.guid()}`;
		(args || []).forEach((ref, i) => {
			if (!("id" in ref)) return;
			const localObj = local;
			localObj[ref.id] = callArgs[i] ? this.getLiteral(callArgs[i]) : void 0;
		});
		return this.eval(asts, local, contextId);
	}
	/**
	* eval
	* @param str {array|string} input string
	* @param local {object} local variable
	* @param contextId {=string} optional contextId, this contextId use to find local variable
	* @return {string}
	*/
	eval(str, local, contextId) {
		if (!local) {
			if (Array.isArray(str)) return this.renderAstList(str);
			return this.evalStr(str);
		}
		const asts = Array.isArray(str) ? str : require_parse.parse(str);
		contextId = contextId ?? `eval:${require_utils.guid()}`;
		if (!asts.length) return "";
		this.local[contextId] = local;
		const ret = this.renderAstList(asts, contextId);
		this.local[contextId] = {};
		this.conditions.shift();
		this.contextId = this.conditions[0] || "";
		return ret;
	}
	/**
	* Evaluate double-quoted strings, replace variables within them, only supporting the most basic variable type replacement
	*/
	evalStr(str) {
		const asts = require_parse.parse(str);
		return this.renderAstList(asts, this.contextId);
	}
	/**
	* parse #foreach
	*/
	getBlockEach(block) {
		const ast = block[0];
		const _from = this.getLiteral({
			...ast.from,
			pos: ast.pos
		});
		const _block = block.slice(1);
		const _to = ast.to;
		const local = {
			foreach: {
				count: 0,
				index: 0,
				hasNext: false
			},
			velocityCount: 0
		};
		let ret = "";
		const contextId = `foreach:${require_utils.guid()}`;
		const type = {}.toString.call(_from);
		if (!_from || type !== "[object Array]" && type !== "[object Object]") return "";
		const items = Array.isArray(_from) ? _from : Object.values(_from);
		const len = items.length;
		items.forEach((val, i) => {
			if (this.runState.break) return;
			const localObj = local;
			localObj[_to] = val;
			localObj.foreach = {
				count: i + 1,
				index: i,
				hasNext: i + 1 < len
			};
			localObj.velocityCount = i + 1;
			this.local[contextId] = local;
			ret += this.renderAstList(_block, contextId);
		});
		if (_from?.length) {
			this.runState.break = false;
			this.local[contextId] = {};
			this.conditions.shift();
			this.contextId = this.conditions[0] || "";
		}
		return ret;
	}
	/**
	* parse #if
	*/
	getBlockIf(block) {
		let received = false;
		const asts = [];
		block.some((ast) => {
			const hasCondition = ast.type === "elseif" || ast.type === "if";
			if (!(hasCondition || ast.type === "else")) {
				received && asts.push(ast);
				return;
			}
			if (received) return true;
			received = hasCondition ? !!this.getExpression(ast.condition) : true;
		});
		return this.renderAstList(asts, this.contextId);
	}
};
//#endregion
exports.BlockCommand = BlockCommand;
