...
function getRefText(ast) {
	if (ast.type === "macro_call") return `#${getMethodText(ast)}`;
	let ret = ast?.leader || "";
	const isFn = ast.args !== void 0;
	if (ast.isWraped) ret += "{";
	if (isFn) ret += getMethodText(ast);
	else ret += ast.id;
	ast.path?.forEach((ref) => {
		...
		if (ref.type === "index") {
			let text = "";
			const id = ref.id;
			if (id.type === "integer") text = "" + id.value;
			else if (id.type === "string") {
				const sign = id.isEval ? "\"" : "'";
				text = sign + id.value + sign;
			} else text = getRefText(id);
			ret += "[" + text + "]";
		}
		if (ref.type === "property") ret += "." + ref.id;
	});
	if (ast.isWraped) ret += "}";
	return ret;
}
